  ...
  <view class="container">
    ...
    <view class="order-tit">
      <text
        @tap="onStsTap"
        data-sts="0"
        :class="[orderStatusClasses(0)]"
      >全部</text>
      <text
        @tap="onStsTap"
        data-sts="1"
        :class="[orderStatusClasses(1)]"
      >待支付</text>
      <text
        @tap="onStsTap"
        data-sts="2"
        :class="[orderStatusClasses(2)]"
      >待发货</text>
      <text
        @tap="onStsTap"
        data-sts="3"
        :class="[orderStatusClasses(3)]"
      >待收货</text>
      <text
        @tap="onStsTap"
        data-sts="5"
        :class="[orderStatusClasses(5)]"
      >已完成</text>
    </view>
    <!-- end 头部菜单 -->
    <view class="main">
        ...
        <view class="prod-item">
          <view class="order-num">
            ...
            <view class="order-state">
              <text
                :class="[orderDetailsState(item.status)]">{{item.status==1?'待支付':(item.status==2?'待发货':(item.status==3?'待收货':(item.status==5?'已完成':'已取消')))}}</text>
                  ...
                  <view class="prod-info">
                    ...
                    <view class="price-nums">
                      <text class="prodprice"><text class="symbol">￥</text>
                        ...
                        <text class="small-num">.{{(prod.price)[1]|parsePrice}}</text></text>
          ...
          <view class="total-num">
            ...
            <view class="prodprice">合计：
              ...
              <text class="big-num">{{(item.actualTotal)[0]|parsePrice}}</text>
              <text class="small-num">.{{(item.actualTotal)[1]|parsePrice}}</text>
            </view>
          </view>
          <!-- end 商品列表 -->
