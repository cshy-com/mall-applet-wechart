<template>
  ...
  <view class="container">
    ...
    >
      ...
      >
        <swiper-item>
          <image :src="require('@/static/img/test5.png')"></image>
        </swiper-item>
      </block>
    </swiper>
    <!-- end  轮播图 -->
    ...
    <view class="prod-info">
      <view class="tit-wrap">
        <view class="prod-tit">{{prodName||'这里是商品的名称这里是商品的名称这里是商品的名称'}}
        </view>
        <view
          class="col"
        ...
        >
          <image
            v-if="!isCollection"
            src="/static/img/icon/prod-col.png"
          ></image>
          <image
            v-if="isCollection"
            src="/static/img/icon/prod-col-red.png"
          ></image>
          收藏
        </view>
      </view>
      <view class="sales-p">{{brief||'这里是简介简介这里是简介简介这里是简介简介这里是简介简介这里是简介简介'}}</view>
      <view class="prod-price">
        <text
          v-if="defaultSku && defaultSku.price"
          class="price"
        >￥<text
            class="price-num">{{(defaultSku.price)[0]|parsePrice}}</text>.{{(defaultSku.price)[1]|parsePrice}}</text>
        <text
          v-if="defaultSku && defaultSku.oriPrice"
          class="ori-price"
        >
          ￥{{(defaultSku.oriPrice)[0]|parsePrice}}.{{(defaultSku.oriPrice)[1]|parsePrice}}
        </text>
    ...
    >
      ...
      <view class="sku-con">{{selectedProp.length>0?selectedProp+'，':''}}{{prodNum||2}}件</view>
    ...
    <view class="cmt-wrap">
      ...
      >
        <view class="cmt-t">
          评价
          <text class="cmt-good">好评{{prodCommData.positiveRating||80}}%</text>
        </view>
        <view class="cmt-count">
          共{{prodCommData.number||1255}}条
      ...
      <view class="cmt-cont">
        <view
          ...
          @tap="showComment"
        >
          <text>全部({{prodCommData.number||1000}})</text>
          <text>好评({{prodCommData.praiseNumber||100}})</text>
          <text>中评({{prodCommData.secondaryNumber||100}})</text>
          <text>差评({{prodCommData.negativeNumber||30}})</text>
          <text>有图({{prodCommData.picNumber||10}})</text>
        </view>
    ...
    <view class="cart-footer">
      <view
        ...
        @tap="toHomePage"
      >
        <image src="/static/img/tab_home_selected.png"></image>
        首页
      </view>
      <view
        class="btn icon"
        @tap="toCartPage"
      >
        <image src="/static/img/tab_shop_unselected.png"></image>
        购物车
        <view
          class="badge badge-1"
          v-if="totalCartNum>0"
      ...
      <view class="pup-sku-main">
        <view class="pup-sku-header">
          ...
          <view class="pup-sku-price">
            ...
            >{{(defaultSku.price)[0]|parsePrice}}</text>.{{(defaultSku.price)[1]|parsePrice}}
          </view>
            ...
            >
              ...
              <view class="sku-choose">
                ...
                >
                  ...
                  <text
                    :class="skuChooseItem"
                    :data-ok="(allProperties,selectedPropObj,key,item,propKeys)|props_contain"
                    @tap="toChooseItem"
                    :data-key="key"
                    :data-val="item"
                  >{{item}}</text>
      ...
      <view class="cmt-cont">
        <view class="cmt-tag">
          <text
            @tap="getProdCommPage"
            data-evaluate="-1"
            :class="[evaluateClass(-1)]"
          >全部({{prodCommData.number}})</text>
          <text
            @tap="getProdCommPage"
            data-evaluate="0"
            :class="[evaluateClass(0)]"
          ...
          <text
            @tap="getProdCommPage"
            data-evaluate="2"
            :class="[evaluateClass(2)]"
          >差评({{prodCommData.negativeNumber}})</text>
          <text
            @tap="getProdCommPage"
            data-evaluate="3"
            :class="[evaluateClass(3)]"
...
export default {
  data() {
    return {
      ...
      price: 0,
      content:
        '这里是商品的详情页面这里是商品的详情页面这里是商品的详情页面这里是商品的详情页面这里是商品的详情页面这里是商品的详情页面',
      prodId: 0,
      brief: '',
      skuId: 0,
      popupShow: false,
      ...
      littleCommPage: [
        {
          recTime: '2023-12-01 12:30:20',
          nickName: '用户昵称',
          content: '很好吃，店铺环境nice',
          pics: []
        },
        {
          recTime: '2023-12-01 12:30:20',
  ...
  computed: {
    skuChooseItem() {
      return 'sku-choose-item'
      return (
        'sku-choose-item ' +
        ((selectedProp, item) | array_contain ? 'active' : '') +
        ' ' +
        ((allProperties, selectedPropObj, key, item, propKeys) | props_contain ? '' : 'gray')
      )
    }
  },
