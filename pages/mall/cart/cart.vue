...
<view class="container">
    ...
    <block v-for="(item, scIndex) in shopCartItemDiscounts" :key="scIndex">
      <view class="prod-block">
        <view class="discount-tips" v-if="item.chooseDiscountItemDto">
          <text class="text-block">{{(item.chooseDiscountItemDto.discountRule) |parseDiscount}}</text>
          <text class="text-list">{{(item.chooseDiscountItemDto.discountRule,item.chooseDiscountItemDto.needAmount,item.chooseDiscountItemDto.discount) |parseDiscountMsg}}</text>
        </view>
              ...
              <view class="opt">
                ...
                <view class="price-count">
                  <view class="price">
                    ...
                    <text class="small-num">.{{(prod.price)[1] |parsePrice}}</text>
                  </view>
  ...
  <view class="empty" v-if="shopCartItemDiscounts.length==0">
    <view class="img">
      <image src="/static/img/home/basket.png"></image>
    </view>
    <view class="txt">
      您还没有添加任何商品哦~
    </view>
  ...
  <view class="cart-footer" v-if="shopCartItemDiscounts.length>0">
    ...
    <view class="btn total">
      <view class="finally">
        ...
        <view class="price">
          <text class="symbol">￥</text>
          <text class="big-num">{{(finalMoney)[0]|parsePrice}}</text>
          <text class="small-num">.{{(finalMoney)[1]|parsePrice}}</text>
        </view>
      </view>
      <view class="total-msg" v-if="subtractMoney>0">
        总额:￥{{(totalMoney) |parsePrice}} 立减:￥{{(subtractMoney) |parsePrice}}
      </view>
    </view>
