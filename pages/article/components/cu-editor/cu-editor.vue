<template>
  <view class="editor-container">
    ...
    <view
      class="fixed-bottom"
      :hidden="!toolbarShow"
      :class="{'fixed-bottom-safe':!inputFocus}"
      :style="{ bottom: fixedBottom + 'px' }"
      @touchstart.stop=""
    >
      <view class="toolbar selector" :style="{ height: toolBarHeight + 'rpx' }">
        <view class="toolbar-item-header toolbar-item" @touchend.stop="save(0)">
          <image
            :src="defDraft"
            :lazy-load="true"
            :lazy-load-margin="0"
        ...
        <view class="toolbar-item" @touchend.stop="formatformat('chooseImage')">
          <image
            :src="defPic"
            :lazy-load="true"
            :lazy-load-margin="0"
        ...
        <view class="toolbar-item-footer">
          <!-- <button class="btn btn-primary" @touchend.stop="save(0)">
            保存
          </button> -->
...
export default {
  ...
  data() {
    return {
      ...
      toolBarHeight: 60, // 工具栏高度
      toolBarContentHeight: 0, // 工具栏内容高度
  ...
  computed: {
    defDraft() {
      return this.$fileUrl + '/sysFile/ic_tab_caogao.png'
    },
    defPic() {
      return this.$fileUrl + '/sysFile/ic_tab_tupian.png'
    },
    defUndo() {
      return this.$fileUrl + '/sysFile/ic_tab_cehui.png'
    },
    defRedo() {
      return this.$fileUrl + '/sysFile/ic_tab_cexiao.png'
    },
    defRetract() {
      return this.$fileUrl + '/sysFile/ic_tab_shouqi.png'
    },
    fullToolBarHeight() {
      let height = 0
    ...
    scrollHeight() {
      return (
        this.scrollHeightDefault -
      
        this.fullToolBarHeight -
        this.btnListHeight
      )
    },
    scrollViewHeight() {
  ...
  mounted() {
    ...
    const system = uni.getSystemInfo({
      success: (e) => {
        ...
        this.scrollHeightDefault =
          e.windowHeight - 34 - this.toolBarHeight-10
      },
    })
  ...
  methods: {
    ...
    onEditorFocus(e) {
      ...
      this.$emit("onFocus",this.scrollHeight)
      console.log('this.keyboardHeight:' + this.keyboardHeight)
      console.log('this.scrollHeightDefault:' + this.scrollHeightDefault)
      ...
      // //   console.log( 'delta:'+JSON.stringify( res.delta))
      // // })
    //   setTimeout(() => {
    //   uni.pageScrollTo({
    //     scrollTop: this.scrollHeightDefault+e.target.offsetTop+this.keyboardHeight, //滚动的距离
    //     duration: 30, //过渡时间
    //   })
    // }, 5000)
      // let view = uni.createSelectorQuery().in(this).select('.cu-editor')
