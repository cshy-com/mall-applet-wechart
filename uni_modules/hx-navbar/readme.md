...
### `uni_modules`方式导入的插件无需注册组件即可使用
 1. 使用 HBuilderX 导入插件 即可
 2. 查看[uni_modules](https://uniapp.dcloud.io/uni_modules)和[easycom](https://uniapp.dcloud.io/collocation/pages?id=easycom)


### 非`uni_modules`使用方式注册
页面使用需在 ``` script ``` 中引用组件
``` javascript
